...
const sharedLinkClass =
  "rounded-full px-3.5 py-2 text-sm font-medium transition hover:bg-fern/10 hover:text-fern md:px-3.5 md:py-1 md:text-sm lg:px-4 lg:py-1.5 lg:text-sm";
...
export function NavBar() {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-[#8eaf99] bg-[#cfe0d4]">
      <div className="mx-auto max-w-6xl px-2.5 py-2 sm:px-4 md:px-5 md:py-2">
          ...
          <div className="flex items-center justify-center xl:justify-self-center">
            <Link to="/home" className="flex items-center gap-2.5">
              <img
                ...
                width="160"
                height="145"
                className="h-9 w-auto object-contain 2xl:h-11"
              />
        ...
        <div className="xl:hidden">
          <div className="flex items-center justify-between gap-3">
            <Link to="/home" className="flex min-w-0 items-center gap-2 sm:gap-2.5">
              <img
                src="/logo-display.png"
                alt="PetNest logo"
                width="160"
            ...
            <button
              ...
              className="inline-flex h-8 w-8 shrink-0 items-center justify-center text-ink transition hover:text-fern sm:h-9 sm:w-9 md:h-10 md:w-10"
              onClick={() => setMobileMenuOpen((open) => !open)}
            >
              <svg viewBox="0 0 24 24" className="h-8 w-8 sm:h-9 sm:w-9 md:h-10 md:w-10" fill="none" stroke="currentColor" strokeWidth="1.7">
                {mobileMenuOpen ? (
                  <>
                    <path d="M6 6l12 12" />
                    <path d="M18 6 6 18" />
          ...
          {mobileMenuOpen ? (
            <div className="mt-2 rounded-[22px] bg-white/85 p-2 text-ink shadow-sm ring-1 ring-black/5 backdrop-blur">
              <div className="flex flex-col gap-2">
                {renderSearchForm("flex items-center gap-2 px-1 pb-1")}
                <NavLink to="/adopt" className={sharedLinkClass}>
                  Adopt
                </NavLink>
                <NavLink to="/browse" className={sharedLinkClass}>
                ...
                <Link to={user ? "/dashboard/favorites" : getProtectedRedirect("/dashboard/favorites")} className={sharedLinkClass}>
                  Favorites
                </Link>
                <Link to={user ? "/dashboard" : "/auth"} className={sharedLinkClass}>
                ...
                {user ? (
                  <button
                    type="button"
                    onClick={() => void signOut()}
                    className="rounded-full bg-ink px-3.5 py-2 text-left text-sm font-medium text-white"
                  >
                    Logout
                  </button>
                ) : null}
